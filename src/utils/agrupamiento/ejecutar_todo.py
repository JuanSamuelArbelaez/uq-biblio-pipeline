...
Ejecución:
    python3 utils/agrupamiento/ejecutar_todo.py
...
def main_dendrograms_analizer():
    ...
    merged_bib = pr / "datos" / "bib" / "consolidado.bib"
    ...
    Z_avg, coph_avg = ejecutar_average_linkage(X, keys, out_dir, p=6, max_label_len=60, orientacion="top")  # Average linkage
    Z_comp, coph_comp = ejecutar_complete_linkage(X, keys, out_dir, p=6, max_label_len=60, orientacion="top")  # Complete linkage
    Z_ward, coph_ward, svd = ejecutar_ward(X, keys, out_dir, n_componentes=2, p=6, max_label_len=60, orientacion="top")  # Ward linkage
    ...
    sil_avg = evaluar_por_silhouette(X, Z_avg, "average", k_range, metrica="cosine")
    # Calcula silhouette para complete linkage
    sil_comp = evaluar_por_silhouette(X, Z_comp, "complete", k_range, metrica="cosine")
    ...
    sil_ward = evaluar_por_silhouette(X, Z_ward, "ward", k_range, metrica="euclidean", X_reducido=X_reduced)
